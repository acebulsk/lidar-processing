...
survey$z_soil[which(survey$z_type=='s')]<-survey$z[which(survey$z_type=='s')]-survey$Hs_insitu[which(survey$z_type=='s')]

#For loop to extract elevation data from UAV data corresponding to survey point data
#call for bare ground assessment
survey$DSM_z_soil<-raster::extract(DSM_stack[[bare_index]],SpatialPoints(cbind(survey$X_utm,  survey$Y_utm)))
...
survey$Hs_lidar<-NA
for(i in names(SD)){
  survey$Hs_lidar[which(survey$Identifier==gsub("X", "",i))]<- extract(SD[[i]],SpatialPoints(cbind(survey$X_utm[which(survey$Identifier==gsub("X", "",i))],  
...
write.csv(errors, 'data/error_summary/error_summary.csv', row.names = F)
