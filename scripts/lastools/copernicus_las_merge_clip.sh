...
for A in "${event_list[@]}"; do
    ...
    pt_cld_path_out="/gpfs/tp/gwf/gwf_cmt/zvd094/fortress/lidar-processing/data/pointclouds_stripalign_clipmerge"

    # Merge LAS files
    $las_path/lasmerge64 -v -i $pt_cld_path/*.las \
        -o $pt_cld_path_out/${A}_sa_mrg_clp.las

    # Clip the merged LAS file
    $las_path/lasclip64 -v -i $pt_cld_path_out/${A}_sa_mrg_clp.las \
        -poly $shp_clip_new \
        -o $pt_cld_path_out/${A}_sa_mrg_clp.las
