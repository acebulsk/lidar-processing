...
df_avg_surv_err <- read.csv('prepost_data/error_summary/pre_post_error_summary.csv') 
...
fls <- list.files('prepost_data/lidar_w_survey_hs/', 
...
df_gnss_lidar_surface_ele |> 
  ...
  geom_point() +
  geom_abline() +
  facet_wrap(~jday_from + Identifier + to, scales = 'free') +
...
df_gnss_lidar_surface_ele |> 
  ggplot(aes(bias*100, x = canopy)) +
  geom_boxplot() +
  facet_grid(~jday_from + Identifier + to) +
